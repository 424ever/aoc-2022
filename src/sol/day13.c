...
static struct aoc_sol day13_sol = {
	.name = "day13",
	.sol  = &day13_sol_func
};
...
void day13_sol_func (FILE *in_f, FILE *out_f, FILE *debug_out)
{
	...
	for (i = 0; i < npackets; ++i)
	{
		if (packets[i] == div_packs[0] ||
		    packets[i] == div_packs[1])
			idx_prod *= i + 1;
		print_packet (packets[i], debug_out);
		fputc ('\n', debug_out);
...
struct packet *parse_packet (char **lineptr)
{
	...
	if (**lineptr == '[')
	{
		...
		for (;;)
		{
			...
			++pack->nvalues;
			pack->values = reallocarray(pack->values, pack->nvalues, sizeof (struct packet*));
...
{
	return compare_packets((struct packet*)*((const void**) a), (struct packet*)*((const void**) b));
}
