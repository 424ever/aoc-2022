...
static void smallest_enough_space_freed(struct dirent *, size_t, size_t,
					size_t *);

static struct aoc_sol day7_sol = {.name = "day7", .sol = &day7_sol_func};
...
void day7_sol_func(FILE *in_f, FILE *out_f, FILE *debug_out)
{
	...
	smallest_enough_space_freed(root, 70000000 - dirent_tot_size(root),
				    30000000, &part2);
...
void dirent_move_child(struct dirent *child, struct dirent *parent)
{
	...
	parent->nchild++;
	parent->children = reallocarray(parent->children, parent->nchild,
		...
		if (parent->children[i] == child)
		{
			if (i != parent->nchild - 1)
				memmove(parent->children + i,
					...
					parent->nchild - 1 - i);
			parent->children =
			    reallocarray(parent->children, parent->nchild - 1,
					 sizeof(struct dirent *));
			--parent->nchild;
			break;
		}
...
void smallest_enough_space_freed(struct dirent *ent, size_t free, size_t needed,
				 size_t *candidate)
{
	size_t i;

	if (ent->dir && needed <= free + dirent_tot_size(ent) &&
	    dirent_tot_size(ent) < *candidate)
		*candidate = dirent_tot_size(ent);
	for (i = 0; i < ent->nchild; ++i)
		smallest_enough_space_freed(ent->children[i], free, needed,
					    candidate);
}
