...
static struct aoc_sol day11_sol = {
	.name = "day11",
	.sol  = &day11_sol_func
};
...
void day11_sol_func (FILE *in_f, FILE *out_f, FILE *debug_out)
{
	...
	{
		perform_round (monkeys1, nmonkeys, throwcounts1, true, debug_out, round == 1);
	...
	{
		perform_round (monkeys2, nmonkeys, throwcounts2, false, debug_out, round == 2);
		
		if (round == 1 || round == 2 || round == 20 || round % 1000 == 0)
		{
			fprintf (debug_out, "== After round %d ==\n", round);
			for (i = 0; i < nmonkeys; ++i)
				fprintf (debug_out, "Monkey %d inspected items %ld times.\n", i, throwcounts2[i]);
		}
	}
	...
	for (;;)
	{
		...
		*mptr = reallocarray (*mptr, *nptr, sizeof (**mptr));
		...
		for (;;)
		{
			...
			{
				fprintf (stderr, "Not enough space for items of %d\n", *nptr - 1);
				abort ();
			}
		}
...
void dump_state (FILE *f, struct monkey *m, int n, bool full)
{
	...
	for (i = 0; i < n; ++i)
	{
		...
		if (full)
		{
			...
			fputc(m[i].op.type == ADD ? '+' : m[i].op.type == MUL ? '*' : '?', f);
			if (m[i].op.rhs == OP_OLD)
				fprintf (f, " %s\n", "old");
			else
				fprintf (f, " %" PRIi64 "\n", m[i].op.rhs);
			fprintf (f, "  Test: %d\n", m[i].test);
			fprintf (f, "    true: %d\n    false: %d\n", m[i].actrue, m[i].acfalse);
		} else {
			for (j = 0; j < m[i].nitems; ++j)
				fprintf (f, "%" PRIi64 " ", m[i].items[j]);
		}
...
void perform_round (struct monkey *m, int n, size_t *tcounts, bool div_value, FILE *debug, bool print_debug)
{
	int i;
	int j;
	int orig_nitems;
	...
	for (i = 0; i < n; ++i)
	{
		...
		for (j = 0; j < orig_nitems; ++j)
		{
			...
			if (div_value)
			{
				m[i].items[0] /= 3;
				if (print_debug)
					fprintf (debug, "    decreased to %" PRIi64 "\n", m[i].items[0]);
			} else
			...
			{
				if (print_debug)
					fprintf (debug, "    passed, threw to %d\n", m[i].actrue);

				throw_item (m, i, m[i].actrue);
			} else
			{
				if (print_debug)
					fprintf (debug, "    not passed, threw to %d\n", m[i].acfalse);
...
void throw_item (struct monkey *m, int from, int to)
{
	...
	memmove (m[from].items, m[from].items + 1, sizeof (m[from].items) - sizeof (*m[from].items));
