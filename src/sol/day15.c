...
static struct aoc_sol day15_sol = {
	.name = "day15",
	.sol  = day15_sol_func
};
...
void day15_sol_func (FILE *in_f, FILE *out_f, FILE *debug_out)
{
	...
	for (i = 0; i < nsensors; ++i)
	{
		...
		for (check_pos.y = sensors[i].pos.y - sensors[i].dist_to_closest - 1;
		     check_pos.y < sensors[i].pos.y + sensors[i].dist_to_closest + 2; ++check_pos.y)
		{
			rem = sensors[i].dist_to_closest - abs (sensors[i].pos.y - check_pos.y) + 1;
...
void print_sensors (FILE *f, struct sensor *s, size_t n)
{
	...
	for (i = 0; i < n; ++i)
	{
		fprintf (f, "%ld: sensor { x: %d y: %d dist: %d }\n", i, s[i].pos.x, s[i].pos.y, s[i].dist_to_closest);
	}
}
