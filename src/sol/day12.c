...
static struct aoc_sol day12_sol = {
	.name = "day12",
	.sol  = &day12_sol_func
};
...
void day12_sol_func (FILE *in_f, FILE *out_f, FILE *debug_out)
{
	...
	while (getline (&line, &nread, in_f) != -1)
	{
		...
		for (i = 0; i < w; ++i)
		{
			heightmap = realloc (heightmap, w * h * sizeof (*heightmap));
			if (line[i] == 'S')
			{
				startx = i;
				starty = h - 1;
	...
	for (y = 0; y < h; ++y)
	{
		for (x = 0; x < w; ++x)
		{
			if (heightmap[y * w + x] == 0)
			{
				...
				mindist = min (mindist, dijkstra(heightmap, h, w, x, y, endx, endy));
			}
		}
	}
...
uint32_t dijkstra (int *map, int h, int w, int startx, int starty, int endx, int endy)
{
	int                   nnodes;
	int                   x, y;
	...
	for (;;)
	{
		...
		if (neighbor && !neighbor->visited)
		{
			...
			neighbor->tenative_dist = neighbor->tenative_dist > newdist ? newdist : neighbor->tenative_dist;
		}
...
struct dijkstra_node *node_at (struct dijkstra_node *nodes, int nnodes, int x, int y)
{
	int i;
